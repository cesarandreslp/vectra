...
interface Resultado { created: number; skipped: number; errors: string[] }

/** Carga masiva de testigos por Excel — plantilla + subida + resultado. */
export function ImportarTestigos() {
  ...
  async function subir(file: File) {
    ...
    try {
      ...
      const r = await fetch('/api/core/importar-testigos', { method: 'POST', body: fd })
  ...
  return (
    <div style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', background: '#f8fafc' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <strong style={{ fontSize: '0.95rem' }}>Importar testigos desde Excel</strong>
      ...
      <p style={{ margin: 0, fontSize: '0.78rem', color: '#64748b', lineHeight: 1.5 }}>
        Columnas: <code>nombre, cedula, fecha_nacimiento, telefono, puesto, mesa, email</code>.
        Obligatorias: <strong>nombre, cédula y fecha de nacimiento</strong> (el testigo debe ser mayor de edad — la fecha es su credencial de acceso).
        <code>puesto</code> + <code>mesa</code> asignan la mesa que vigila (si la mesa ya tiene testigo, esa fila se rechaza). El correo es opcional.
      </p>

      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <a href="/api/core/plantilla-testigos"
      ...
      {res && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#166534' }}>
            ✓ {res.created} testigo(s) creado(s){res.skipped ? ` · ${res.skipped} omitido(s) (ya existían)` : ''}
          </div>
          {res.errors.length > 0 && (
          ...
          <span style={{ fontSize: '0.72rem', color: '#94a3b8' }}>Recargá la página para ver la lista y la cobertura actualizadas.</span>
        </div>
      )}
    </div>
  )
